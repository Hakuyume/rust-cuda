...
fn vector_add<A, B, C>(grid_dim: cuda::Dim3,
                       block_dim: cuda::Dim3,
                       a: &cuda::memory::ArrayBase<A>,
                       b: &cuda::memory::ArrayBase<B>,
                       c: &mut cuda::memory::ArrayBase<C>,
                       num: usize,
                       stream: &cuda::Stream)
                       -> cuda::Result<()>
    where A: cuda::memory::Repr<Type = c_float>,
          B: cuda::memory::Repr<Type = c_float>,
          C: cuda::memory::ReprMut<Type = c_float>
{
    assert_eq!(a.len(), num);
    assert_eq!(b.len(), num);
    assert_eq!(c.len(), num);
    ...
    unsafe {
        cuda::launch_kernel(vectorAdd as *const c_void,
                            grid_dim.into(),
                            block_dim.into(),
                            &mut [&mut a.as_ptr(),
                                  &mut b.as_ptr(),
...
fn main() {
    ...
    cuda::memcpy(&mut d_a, &h_a).unwrap();
    cuda::memcpy(&mut d_b, &h_b).unwrap();
    ...
    cuda::memcpy(&mut h_c, &d_c).unwrap();
